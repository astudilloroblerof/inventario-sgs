...
class Home {
	...
	public function get_checked_out($interval) {
		if($interval == 'today')
			$q = "SELECT SUM(fromqty - toqty) as s FROM invento_logs WHERE `type`=1 AND DATE(date_added) = DATE(NOW())";
		elseif($interval == 'this_week')
...
//////// CONTEO DE NUEVOS, ENTRADA Y SALIDAS PARA LA PAGINA DE INICIO

		public function get_new_items1($interval) {
		if($interval == 'today')
			$q = "SELECT count(*) as c FROM invento_items WHERE category = '2' AND DATE(date_added) = DATE(NOW())";
		elseif($interval == 'this_week')
			$q = "SELECT count(*) as c FROM invento_items WHERE category = '2' AND date_added > DATE_SUB(NOW(), INTERVAL 1 WEEK)";
		elseif($interval == 'this_month')
			$q = "SELECT count(*) as c FROM invento_items WHERE category = '2' AND date_added > DATE_SUB(NOW(), INTERVAL 1 MONTH)";
		elseif($interval == 'this_year')
			$q = "SELECT count(*) as c FROM invento_items WHERE category = '2' AND date_added > DATE_SUB(NOW(), INTERVAL 1 YEAR)";
		elseif($interval == 'all_time')
			$q = "SELECT count(*) as c FROM invento_items";
	...
	public function get_checked_in1($interval) {
		if($interval == 'today')
			$q = "SELECT SUM(toqty - fromqty) as s FROM invento_logs WHERE `type`=1 AND DATE(date_added) = DATE(NOW())";
		elseif($interval == 'this_week')
			$q = "SELECT SUM(toqty - fromqty) as s FROM invento_logs WHERE `type`=1 AND date_added > DATE_SUB(NOW(), INTERVAL 1 WEEK)";
	...
	public function get_checked_out1($interval) {
		if($interval == 'today')
			$q = "SELECT SUM(fromqty - toqty) as s FROM invento_logs WHERE `type`=1 AND DATE(date_added) = DATE(NOW())";
		elseif($interval == 'this_week')
			$q = "SELECT SUM(fromqty - toqty) as s FROM invento_logs WHERE `type`=2 AND date_added > DATE_SUB(NOW(), INTERVAL 1 WEEK)";
...
/// FUNCIONES PARA EL CALCULO DE FILTROS, PARA EL ANALISIS DE INICIO

		public function filtros() {
		$res = $this->query("SELECT SUM(qty) as s FROM invento_items WHERE category = 3", 'filtros()');
		$obj = $res->fetch_object();
		if($obj->s == '')
			return 0;
		return $obj->s;
...
/// FUNCIONES PARA EL CALCULO DE repuestos, PARA EL ANALISIS DE INICIO

		public function repuestos() {
		$res = $this->query("SELECT SUM(qty) as s FROM invento_items WHERE category = 2", 'repuestos()');
		$obj = $res->fetch_object();
		if($obj->s == '')
			return 0;
		return $obj->s;
...
/// FUNCIONES PARA EL CALCULO DE INSUMOS CALIBRACIONES, PARA EL ANALISIS DE INICIO

		public function insumos() {
		$res = $this->query("SELECT SUM(qty) as s FROM invento_items WHERE category = 4", 'insumos()');
		$obj = $res->fetch_object();
		if($obj->s == '')
			return 0;
		return $obj->s;
