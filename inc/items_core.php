...
/// OBTENER LISTADO DE FILTROS
	...
	public function get_items($page, $items_per_page, $hoja) {
		$page = stripslashes($page);
		$items_per_page = stripslashes($items_per_page);
		$hoja = stripslashes($hoja);
		...
		if ($hoja == 1) {
			...
			$res = $this->query("SELECT * FROM invento_items WHERE category ='3' ORDER BY id DESC LIMIT $x,$y", 'get_items()');
		return $res;
		}
		elseif ($hoja == 2) {
			$res = $this->query("SELECT * FROM invento_items WHERE category ='2' ORDER BY id DESC LIMIT $x,$y", 'get_items()');
		return $res;
		}
		else{
			$res = $this->query("SELECT * FROM invento_items WHERE category ='4' ORDER BY id DESC LIMIT $x,$y", 'get_items()');
		return $res;
		}
		
	}
		public function get_items1($page, $items_per_page) {
		$page = stripslashes($page);
		$items_per_page = stripslashes($items_per_page);
		...
		$res = $this->query("SELECT * FROM invento_items ORDER BY id DESC LIMIT $x,$y", 'get_items1()');
		return $res;
	}
...
/// OBTENER CATEGORIAS DE LOS ITEMS
...
/// ACTUALIZAR ITEMS
	...
	public function update_item_qty($type, $id, $fromqty, $qty) {
		...
		// INTENCA CREAR EL LOG, SI FALLA, LO REVIERTE
		if($type == 1)
...
/// CREA EL LOGO
	...
	public function get_item2($itemid) {
		$prepared = $this->prepare("SELECT * FROM invento_items WHERE id=?", 'get_item2()');
		$this->bind_param($prepared->bind_param('i', $itemid), 'get_item2()');
		$this->execute($prepared, 'get_item2()');
